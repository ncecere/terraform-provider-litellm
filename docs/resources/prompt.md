...
Ordinary resource refresh uses bounded, cancellable retries for transient failures from the explicit-environment singular read. LiteLLM v1.98 can return HTTP 400 or 404 from either Prisma-backed lookup or its process-local registry, and the versions route does not identify which backend produced a 404. Ordinary refresh therefore retains Terraform state on every ambiguous absence response. Malformed or exhausted reads also retain the prior public and private state. Create/update confirmation and prompt data sources keep their endpoint-specific request behavior rather than inheriting these ordinary-refresh retries.
