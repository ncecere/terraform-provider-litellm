...
Ordinary resource refresh uses bounded, cancellable retries for transient failures from the explicit-environment singular read. LiteLLM v1.98 can return HTTP 400 or 404 for more than one condition, so neither response removes Terraform state by itself. The provider performs one direct environment-scoped version-history read and removes state only when that route returns an exact HTTP 404. Malformed, ambiguous, or exhausted reads retain the prior public and private state. Create/update confirmation and prompt data sources keep their endpoint-specific request behavior rather than inheriting these ordinary-refresh retries.
