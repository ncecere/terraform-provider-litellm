...
- `delegate_auth_to_upstream` - (Bool, Computed) Lets clients complete authentication directly with an upstream OAuth2 MCP server. `true` requires `auth_type = "oauth2"` and cannot be combined with a known `client_credentials` flow. Removing owned configuration restores `false`.
...
- `max_concurrent_requests` - (Int64, Computed) Positive outbound request limit. Configure 1–2147483647; omission/null means unlimited. Non-positive alternate representations of unlimited are rejected to keep one canonical state.
