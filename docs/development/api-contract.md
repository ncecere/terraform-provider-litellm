...
URL construction and query review are inventory-based. Dynamic path values must use the exact package-local `endpointWithPathSegment` or `endpointWithPathCapture` builder. Calls require literal, unqueried prefix and suffix boundaries; aliases, higher-order calls, interfaces, generic callers, reflection, containers, selectors, and dynamic boundaries fail closed. The exact path builders each perform one direct `url.PathEscape` on their raw dynamic parameter, and `endpointWithQuery` performs the sole direct `url.Values.Encode` on raw query values. `url.PathEscape`, `url.QueryEscape`, and `url.Values.Encode` calls, aliases, method values, and higher-order references outside those exact internals are rejected, including preescaped builder arguments and query leaves. Ordinary FastAPI path-segment identities containing a decoded slash fail locally before dispatch with content-free diagnostics; complete ordinary `.` and `..` segments remain hardened. The separately named capture builder is restricted to LiteLLM v1.98's reviewed `{credential_name:path}` routes and permits slash, but rejects any complete `.` or `..` component before dispatch so intermediary decoding and normalization cannot change credential identity. The exact `/fallback/{model}` call is the sole statically restricted ordinary-segment slash exception, preserving merged #206's exact-once escaping and LiteLLM v1.98 route-level 404 contract. `url.Values.Set` and `Add` are identified from their `go/types` method object after alias and pointer dereference and must be direct calls with literal keys; bound methods, method expressions, and higher-order passing/returns fail. URL-value literal and index keys must also be literals. The exact `addKnownStringFilter` definition is the sole dynamic `Set`-key exception, and every call to it must supply a literal key; `cloneURLValues` is the sole dynamic-index copy exception. Passing raw `url.Values` is limited to the exact reviewed helpers `endpointWithQuery`, `cloneURLValues`, `safeListDiagnostic`, `addKnownStringFilter`, `listKeys`, and `listUsers`. `endpointWithQuery` rejects a path that already owns a query or fragment, then uses canonical `url.Values.Encode` output. Pointer, alias, parameter, return, generic, and cross-file mutations therefore either contribute a static key to inventory or fail closed. The verifier then extracts every non-test provider call and compares method, normalized path boundaries, path modes, path parameters, and exact query names with OpenAPI plus the supplement.
