...
- **`litellm_credential` resource and data source**: Preserve the legacy `map(string)` types while loosening `credential_values` to optional for true model-only configuration and source-free imports; add canonical heterogeneous JSON objects; reject v1.98-invalid empty values-only creates; and align source dominance, exact routes, masks, recursive ownership, remote shape drift, import safety, and response validation with LiteLLM v1.98.0. Credential lookups now use bounded fresh-connection worker sampling so one process-local cache's exact 404 cannot remove state or permit duplicate create; mixed cached versions retain identity with an actionable convergence diagnostic, and delete requires consecutive all-404 sampling. ([#192](https://github.com/ncecere/terraform-provider-litellm/issues/192))
