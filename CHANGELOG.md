...
- Baseline clients are Terraform >= 1.0.0 and OpenTofu >= 1.6.0. Only configurations using optional write-only `key_wo` or key/user `send_invite_email` require Terraform or OpenTofu >= 1.11.0. Provider development requires Go >= 1.24.0, and the tested backend is exactly LiteLLM 1.98.0. Published examples constrain the provider to `>= 2.0.1, < 3.0.0`. ([#204](https://github.com/ncecere/terraform-provider-litellm/issues/204))
...
- **Provider distribution and compatibility**: Serve the exact published `registry.terraform.io/ncecere/litellm` source, centralize compatibility metadata, align installation examples on provider `>= 2.0.1, < 3.0.0`, add semantic HCL/manifest/address contracts, and harden GoReleaser v2 and exact-tag CHANGELOG release-note automation. This changes no protocol, schema, HCL type, state value, ID, import, or normal canonical state. ([#204](https://github.com/ncecere/terraform-provider-litellm/issues/204))
