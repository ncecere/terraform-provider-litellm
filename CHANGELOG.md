...
### Upgrade Notes
- The published provider source remains `registry.terraform.io/ncecere/litellm`. Correcting the executable's served address does not change protocol 6, schemas, HCL types, state values, IDs, imports, or normal canonical state, so ordinary users do not need a state migration. Development-only state actually recorded under the unpublished `registry.terraform.io/nicholas-cecere/litellm` address must be migrated explicitly with `terraform state replace-provider registry.terraform.io/nicholas-cecere/litellm registry.terraform.io/ncecere/litellm` (or the equivalent `tofu` command); no silent address alias is provided. ([#204](https://github.com/ncecere/terraform-provider-litellm/issues/204))
- The Terraform minimum increases from 1.0.0 to 1.1.0. Terraform 1.0 can make omitted maps and explicit empty maps byte-identical before provider dispatch when `ignore_changes` overlays refreshed state, so the provider cannot preserve both ownership meanings safely on that client. Upgrade Terraform before using this provider release. OpenTofu >= 1.6.0 remains supported. Only configurations using optional write-only `key_wo` or key/user `send_invite_email` require Terraform or OpenTofu >= 1.11.0. Provider development requires Go >= 1.24.0, and the tested backend is exactly LiteLLM 1.98.0. Published examples constrain the provider to `>= 2.0.1, < 3.0.0`. ([#139](https://github.com/ncecere/terraform-provider-litellm/issues/139), [#204](https://github.com/ncecere/terraform-provider-litellm/issues/204))

### Security
- **Key lookup metadata sensitivity (issue #202)**: Mark `litellm_key` data-source metadata sensitive, matching the resource and preventing credential-bearing metadata from appearing in normal CLI output. Malformed key-hash validation diagnostics no longer reflect the configured identifier. Existing HCL types and state values are unchanged. ([#202](https://github.com/ncecere/terraform-provider-litellm/issues/202))
- **MCP configuration sensitivity (issue #215)**: Mark MCP server URLs, specification paths, stdio commands/arguments/environments, static headers, and OAuth endpoint URLs sensitive across the resource and singular/list data sources without changing their HCL types or state values. Existing outputs derived from these attributes must opt into sensitivity. Extra header names, access groups/tools, audit metadata, transport, and the safe `upstream_resource` projection remain non-sensitive. ([#215](https://github.com/ncecere/terraform-provider-litellm/issues/215))
- Release automation now fails closed unless the pushed tag has one exact, nonempty CHANGELOG section with explicit Upgrade Notes and Security subsections. CI uses pinned GoReleaser v2 tooling for nonpublishing configuration and all-target snapshot validation, release builds no longer mutate module files, and the imported signing key must match the reviewed Terraform Registry fingerprint before GoReleaser runs. No provider schema, credential handling, or resource lifecycle behavior changes as part of #204. ([#204](https://github.com/ncecere/terraform-provider-litellm/issues/204))

### Added
- **Canonical MCP issuer and token exchange (issue #208)**: Add resource-only typed `issuer`, `token_exchange_endpoint`, `audience`, `subject_token_type`, and `token_exchange_profile` fields with role-masking retention, schema v8 upgrades, exact null clears, Entra OBO prerequisites, and compatibility-safe atomic migration from LiteLLM's four legacy `credentials` aliases. ([#208](https://github.com/ncecere/terraform-provider-litellm/issues/208))
- **Sensitive MCP environment variables (issue #208)**: Add resource-only ordered `env_vars` definitions with global or per-user scope, encrypted global-value round trips, exact empty-list clears, strict identifier/duplicate validation, role-masked null retention, transactional projection, and schema v7 direct upgrades. Data sources intentionally omit this field because full-admin management responses contain decrypted global secrets. ([#208](https://github.com/ncecere/terraform-provider-litellm/issues/208))
- **Advanced MCP runtime controls (issue #208)**: Add presence-aware resource lifecycle and singular/list data-source parity for upstream authentication delegation, OAuth passthrough, DCR bridging, BYOK metadata, source URLs, positive finite timeouts, and bounded positive concurrency. Schema v6 upgrades every prior MCP state directly; imports adopt observable API values without inferring Terraform ownership, and removals emit LiteLLM v1.98's exact false, null, or empty-list sentinels. Unsafe auth combinations and implicit DCR clears fail before PUT. ([#208](https://github.com/ncecere/terraform-provider-litellm/issues/208))
- **MCP v1.98 field parity (issue #178)**: Add resource lifecycle support for sensitive write-only OAuth scopes, public-internet classification, stored OAuth flow, instructions, and tool display/description maps, with observable singular/list data-source parity. Presence-aware ownership preserves false and empty values, emits exact clear sentinels, retains masked scopes, adopts LiteLLM-stamped flows, blocks unsafe credential/auth transitions, and upgrades prior MCP state directly without changing existing attributes, identities, or imports. Thanks @ripiomatiascalvo for the field discovery and initial implementation reference in #129. ([#178](https://github.com/ncecere/terraform-provider-litellm/issues/178), [#129](https://github.com/ncecere/terraform-provider-litellm/pull/129))
- **Lossless team metadata JSON (issue #218)**: Add sensitive resource-only `litellm_team.metadata_json` for exact heterogeneous metadata while preserving the legacy `map(string)` attribute, generated/custom IDs, imports, dedicated fields, and data sources. Exact create recovery, callback-ciphertext safeguards, server-owned member-budget protection, recursive value-free ownership, fresh complete-column hydration, API-sibling preservation, verified removals, and import non-adoption provide fail-closed lifecycle behavior on LiteLLM v1.98. ([#218](https://github.com/ncecere/terraform-provider-litellm/issues/218))
- **Lossless project metadata JSON (issue #218)**: Add sensitive resource-only `litellm_project.metadata_json` for exact heterogeneous metadata while preserving the legacy `map(string)` attribute, computed-only public ID, imports, and dedicated metadata fields. Provider-generated caller identity, exact team/shared-budget recovery, recursive value-free ownership, fresh complete-column hydration, API-sibling preservation, verified removals, and import non-adoption provide fail-closed lifecycle behavior on LiteLLM v1.98. ([#218](https://github.com/ncecere/terraform-provider-litellm/issues/218))
- **Lossless organization metadata JSON (issue #218)**: Add sensitive resource-only `litellm_organization.metadata_json` for exact heterogeneous metadata while preserving the legacy `map(string)` attribute. Caller-selected create recovery, recursive value-free ownership, fresh complete-column hydration, API-sibling preservation, verified shape removals, and import non-adoption provide fail-closed lifecycle behavior on LiteLLM v1.98. ([#218](https://github.com/ncecere/terraform-provider-litellm/issues/218))
- **MCP v1.98 read parity (issue #215)**: Add a strict singular-only non-sensitive `upstream_resource` projection from LiteLLM's exactly redacted credential object, expand manager-list items with the available access-group, tool, stdio, header, and OAuth fields, and add role-aware null handling for sanitizer-fabricated empty collections and the restricted `mcp_info` sentinel. The MCP resource now exports `updated_at` and `updated_by`; schema v4 upgrades v0-v3 directly while preserving prior ownership generations and lifecycle state. ([#215](https://github.com/ncecere/terraform-provider-litellm/issues/215))
- **Lossless model parameter JSON (issue #218)**: Add sensitive `litellm_model.additional_litellm_params_json` for exact observable heterogeneous custom `litellm_params` values without changing the legacy `map(string)` attribute. Recursive ownership, endpoint-accurate mask recovery, fresh identity-bound update hydration, and safe replacement prevent API-only adoption, masked-secret replay, and shallow-merge retention; values whose type `/model/info` irreversibly stringifies are rejected before mutation. Imports and upgraded states remain unmanaged, and model-budget fields stay reserved for #223. ([#218](https://github.com/ncecere/terraform-provider-litellm/issues/218))
- **Lossless model information JSON (issue #218)**: Add sensitive `litellm_model.additional_model_info_json` for exact heterogeneous custom `model_info` values without changing the legacy `map(string)` attribute. Recursive ownership excludes API-derived metadata, overlap fails before mutation, imports remain unmanaged, and semantic changes use safe replacement. ([#218](https://github.com/ncecere/terraform-provider-litellm/issues/218))
- **Lossless key dictionaries (issue #218)**: Add sensitive resource-only `litellm_key.metadata_json`, `config_json`, and `permissions_json` siblings for heterogeneous native JSON while preserving all legacy `map(string)` attributes. Caller-selected create identity, recursive private ownership, exact-number checks, callback-ciphertext recovery, fresh whole-root hydration, verified removals, import non-adoption, and uncertain-mutation state retention prevent silent coercion, API-sibling loss, and orphaned ownership. ([#218](https://github.com/ncecere/terraform-provider-litellm/issues/218))
- **`litellm_mcp_server` caller-selected identity and nullable names (issue #214)**: Allow an optional create-only `server_id` with replacement semantics while preserving generated-ID state and imports, allow unnamed servers to use alias/ID fallback, and mirror LiteLLM v1.98 name/alias validation plus canonical space-to-underscore alias normalization without drift. ([#214](https://github.com/ncecere/terraform-provider-litellm/issues/214))
- **`litellm_mcp_server` complete MCP info lifecycle (issue #213)**: Add sensitive whole-document JSON, recursive selective overrides, canonical RFC 6901 clear paths, and a non-sensitive ownership generation without changing existing HCL/state/import IDs or the fixed block. Create and Update now resolve and send complete documents, preserve arbitrary nested API fields and exact numbers, hydrate and verify through the direct singular endpoint, retain prior state on uncertain writes, support equal-value takeover without PUT, and commit value-free v2 provenance only after confirmation. Read/import reconcile complete JSON while projecting only exactly owned fixed leaves. Singular and list data sources expose computed sensitive `mcp_info_json`. ([#213](https://github.com/nicholas-cecere/terraform-provider-litellm/issues/213))
- **`litellm_jwt_key_mapping` resource and singular/list data sources**: Manage LiteLLM v1.98 JWT string claim-to-existing-virtual-key mappings with canonical UUID identity/import, write-only key creation, pre-mutation rejection of unverifiable same-identity rotation, credential-gated immutable claim replacement, keyless-import and unknown-plan destroy safety, nullable description ownership/clear, explicit deactivate/reactivate, strict object/envelope validation, singular exact-404 absence proof, bounded double-scan pagination, source-valid empty claims, and redacted single-attempt failure recovery. ([#250](https://github.com/ncecere/terraform-provider-litellm/issues/250))
- **`litellm_key_block`**: Add a hash-only identity mode while retaining the existing sensitive `key` input for backward compatibility. ([#182](https://github.com/ncecere/terraform-provider-litellm/issues/182))
...
### Changed
- **Prompt read reliability (issue #202)**: Ordinary `litellm_prompt` resource refreshes now use bounded, cancellable retries for safe transient failures while preserving explicit environment scope. Successful responses project transactionally. LiteLLM v1.98 does not identify whether prompt 400/404 responses came from Prisma or its process-local registry, so ordinary refresh retains state on every ambiguous absence response instead of orphaning DB/config prompts. Create/update confirmation, data sources, list reads, absence probes, and mutations retain their existing single-attempt or endpoint-specific convergence behavior, and response-body text never establishes absence.
- **Guardrail read reliability (issue #202)**: Ordinary `litellm_guardrail` resource refreshes and singular data-source reads now use bounded, cancellable retries for safe transient failures. Resource refresh requires exact database-backed authority and complete transactional projection, while the observational data source accepts known database/config definitions. Exact typed 404 alone removes resource state; create/update confirmation, list reads, and mutations remain single-attempt, and DELETE no longer treats body text as absence.
- **Key read reliability (issue #202)**: Ordinary `litellm_key` resource refreshes, singular data-source reads, and `litellm_key_block` refreshes now use bounded, cancellable retries for safe transient `/key/info` failures. Successful responses require a complete identity-matched envelope and project transactionally; key-block state is removed only after exact 404 or authoritative `blocked=false`. Create/update confirmation, semantic recovery and hydration, router convergence, invitation/access-group probes, list reads, and mutations remain single-attempt.
- **User read reliability (issue #202)**: Ordinary `litellm_user` resource refreshes and singular data-source reads now use bounded, cancellable retries for safe transient read failures. Exact complete 404 remains terminal, successful responses require matching root/nested identities and project transactionally, and create/update/adoption confirmation, team-convergence probes, invitation preflight, list reads, and all mutations remain single-attempt.
- **Access Group read reliability (issue #202)**: Ordinary `litellm_access_group` resource refreshes and singular data-source reads now use bounded, cancellable retries for safe transient read failures. Exact complete 404 remains terminal, successful responses project transactionally, and create/update confirmations, list reads, and all mutations remain single-attempt.
- **Search Tool read reliability (issue #202)**: Ordinary `litellm_search_tool` resource refreshes and singular data-source reads now use bounded, cancellable retries for safe transient read failures. Exact complete 404 remains terminal, resource refresh removes only authoritatively absent state, and create/update confirmations, list reads, and all mutations remain single-attempt.
- **Heterogeneous dictionary foundation (issue #218)**: Add exact semantic-object parsing, canonical RFC 6901 ownership paths, value-free private provenance, atomic overlay/removal, and fail-closed masked-value recovery for later compatibility-safe JSON attributes. This foundation-only slice does not change public schemas, HCL, state, IDs, imports, or active request behavior. ([#218](https://github.com/ncecere/terraform-provider-litellm/issues/218))
- **Collection conversion safety (issue #224)**: Terraform and API collections are now converted atomically with propagated, content-safe diagnostics and context cancellation. Null, unknown, malformed, or partially decodable elements cannot produce partial mutations or state, while existing schemas, clear semantics, ownership, imports, and masking behavior remain unchanged. ([#224](https://github.com/ncecere/terraform-provider-litellm/issues/224))
- **HTTP reliability foundation**: Add content-free typed HTTP failure classification, exact typed-status helpers, bounded `Retry-After` parsing, mutation dispatch/acceptance uncertainty metadata, and a cancellable GET/HEAD-only safe-read retry API. This is foundation-only phase 1 of #202: no resource or data-source call sites are migrated to retries; existing call paths remain single-attempt, and resource loops, sample counts, and mutation dispatch behavior are unchanged. ([#202](https://github.com/ncecere/terraform-provider-litellm/issues/202))
- **Development and release API contract**: Pin reproducible LiteLLM v1.98.0 OpenAPI provenance, fail-closed lazy and hidden route export, an offline Go AST verifier for every provider HTTP operation, reviewed unsupported durable inventories, and CI/release gates. The provider runtime remains HTTP API-only and does not embed Python, upstream source, or contract artifacts. ([#209](https://github.com/ncecere/terraform-provider-litellm/issues/209))
- **Provider distribution and compatibility**: Serve the exact published `registry.terraform.io/ncecere/litellm` source, centralize compatibility metadata, require Terraform >= 1.1.0 while retaining OpenTofu >= 1.6.0, align installation examples on provider `>= 2.0.1, < 3.0.0`, add semantic HCL/manifest/address contracts, and harden GoReleaser v2 and exact-tag CHANGELOG release-note automation. The Terraform client floor is the only compatibility change; protocol, schema, HCL types, state values, IDs, imports, and normal canonical state remain unchanged. ([#139](https://github.com/ncecere/terraform-provider-litellm/issues/139), [#204](https://github.com/ncecere/terraform-provider-litellm/issues/204))
- **Provider internals**: Remove the unregistered top-level Terraform Plugin SDKv2 implementation and its dependency. The provider binary has registered only the Plugin Framework implementation since the migration, so provider addresses, HCL type names, schemas, state, IDs, and imports are unchanged and require no migration. ([#205](https://github.com/ncecere/terraform-provider-litellm/issues/205))
...
- **Development and CI**: Add read-only build/vet/format/race-test CI with coverage and JUnit artifacts, broaden provider unit coverage, make local provider installation architecture-aware, and add an isolated, failure-preserving LiteLLM v1.98.0 smoke/acceptance harness covering 23 non-enterprise resources. ([#145](https://github.com/ncecere/terraform-provider-litellm/issues/145)) — thanks @msabramo

### Fixed
- **`litellm_mcp_server` presence-aware clears (issue #212)**: Preserve every existing schema flag/type and import projection while adding value-free, digest-bound private ownership for the 14 existing removable MCP fields. Create now transmits explicitly configured empty/false values; Update hydrates through the direct endpoint, sends only changed owned values and exact v1.98 removal sentinels, rejects credential key deletion and unsafe URL/auth implicit clears before PUT, and verifies changed values plus visible unmanaged preservation before committing ownership. Schema v3 upgrades directly from v0/v1/v2; ambiguous legacy removals use a documented two-step ownership migration. ([#212](https://github.com/nicholas-cecere/terraform-provider-litellm/issues/212))
- **`litellm_team` response authority**: Decode tags, guardrails, prompts, per-model limits, limit types, model aliases, and member defaults from their exact LiteLLM v1.98 nested relations; preserve selective metadata ownership and null/empty semantics; and reject malformed or ambiguous read-backs without publishing planned or partial state. Public schema, HCL, state, IDs, and imports are unchanged. ([#217](https://github.com/ncecere/terraform-provider-litellm/issues/217))
- **`litellm_fallback` deletion authority**: Confirm remote absence after every DELETE, including a DELETE 404, and retain prior Terraform state with a bounded secret-safe error when LiteLLM still returns the fallback. Authoritatively retained presence now has a dedicated content-safe diagnostic distinct from timeout, cancellation, connectivity, malformed-response, and other operational confirmation failures. This prevents pinned LiteLLM v1.98's non-authoritative DELETE behavior from silently orphaning active routing configuration. Successful deletion remains dependent on the upstream endpoint contract tracked in LiteLLM #38425. ([#210](https://github.com/ncecere/terraform-provider-litellm/issues/210), [#264](https://github.com/ncecere/terraform-provider-litellm/issues/264))
- **`litellm_mcp_server` import projection**: Track versioned private, exact-leaf provenance separately for Terraform-owned nested strings/costs and API-owned imported costs; keep unconfigured display metadata out of import and legacy state projection; preserve omitted API-owned cost shells without drift; and reconcile authoritative child null, omission, reappearance, and role-sanitized parents without inferring ownership from public values. ([#254](https://github.com/ncecere/terraform-provider-litellm/issues/254))
- **`litellm_agent` lifecycle**: Require a valid create response identity before read-back; retain only confirmed identity on committed-but-unconfirmed creates and prior state on unconfirmed updates; classify absence by exact HTTP 404; make delete idempotent only on exact 404; verify complete stable post-mutation state; preserve import/API ownership; and use v1.98 `PATCH` semantics for supported limit, header, object-permission, agent-card, capability, provider, skill, and collection clears while rejecting API-unsafe phantom clears before mutation. ([#198](https://github.com/ncecere/terraform-provider-litellm/issues/198))
- **`litellm_fallback` resource and data source**: Parse composite imports by the exact LiteLLM v1.98 fallback-type suffix from the right, preserving colon-bearing and other special model identities while retaining existing state IDs. Preserve model-only `general` imports while requiring valid explicit suffixes for colon-bearing identities, escape raw model path values exactly once across read/delete/data-source calls, validate response identity and shape, and keep lifecycle diagnostics content-safe. ([#206](https://github.com/ncecere/terraform-provider-litellm/issues/206))
- **`litellm_mcp_server`**: Align HTTP/SSE URL-or-OpenAPI-spec and URL-less stdio configuration, required stdio command/args checks, built-in command allowlist, auth enum, payloads, explicit endpoint clears/transitions, authoritative post-update verification, response validation, escaped identity paths, imports, and single/list data sources with LiteLLM v1.98. Preserve the existing `url`, `spec_version`, and `skip_url_validation` types/state while deprecating the two unsupported phantom fields, preserving unchanged historical values for upgrade/update/destroy, and rejecting unsafe new or changed non-default use. ([#200](https://github.com/ncecere/terraform-provider-litellm/issues/200))
- **`litellm_agent.object_permission.mcp_tool_permissions`**: Preserve the public `map(string)` contract while validating each value as a JSON string array, sending native arrays to LiteLLM v1.98, preserving semantically equivalent configured spelling, canonicalizing imports and repaired legacy state, transmitting empty-map/empty-array clears, treating API null/omission as authoritative absence without destabilizing successful empty clears, and failing closed on present malformed API values with redacted diagnostics. ([#197](https://github.com/ncecere/terraform-provider-litellm/issues/197))
- **Semantic JSON attributes**: Validate exact root and nested shapes before mutation; preserve semantically equivalent configured spelling and exact numbers; support LiteLLM v1.98 structured guardrail modes; normalize budget aliases without drift; distinguish empty objects from null; and fail closed on malformed budget/search-tool response values without exposing configured content in diagnostics. ([#196](https://github.com/ncecere/terraform-provider-litellm/issues/196))
- **`litellm_tag` resource and data sources**: Read IDs, limits, duration, and model budgets authoritatively from v1.98's nested `litellm_budget_table`; preserve exact integers and import/upgrade omission provenance; expose resource/single/list parity; initialize and clear reset schedules; address both sets and supported clears through the exact verified budget ID; reject shared-budget reassociation, malformed responses, unsupported empty/model-budget clears, and visible duplicate creates; retain mutation-blocked uncertainty state when an errored create becomes readable or its create-time budget association changes, and persist retry state for failed reset initialization. ([#195](https://github.com/ncecere/terraform-provider-litellm/issues/195))
- **`litellm_organization` / `litellm_project`**: Read budget IDs, hard/soft budgets, duration, TPM/RPM, concurrency, and representable per-model controls authoritatively from v1.98's nested `litellm_budget_table` across resources and single/list data sources; preserve exact imported integers and per-field omission provenance; validate relation/envelope identities; initialize and clear reset schedules; route non-null project budget changes through parent-team validation before any explicit reset/clear patch, with partial-failure safety; use transactional organization metadata replacement for removable per-model rate limits; and reject unsafe shared-budget creation, reassociation, or non-convergent clears. Structured `model_max_budget` data-source/organization surfaces remain deferred; the legacy project `map(float64)` field preserves existing state while rejecting inaccurate additions. Organization `blocked` and `tags` remain deprecated schema-compatible fields, but v1.98 phantom writes/reads are ignored and new unsupported non-default values are rejected. ([#188](https://github.com/ncecere/terraform-provider-litellm/issues/188))
- **`litellm_team_member`**: Accept `user_id`, `user_email`, or both without changing existing state/address/import contracts; persist canonical IDs for email-only membership; keep team/canonical-user identity replacement-safe; preserve case-insensitive configured email aliases without managing account email; reconcile exact v1.98 roster, optional budgetless membership, role, and native member-budget state; omit unchanged budget fields on role-only updates; retain accepted uncertain/partial ownership without publishing requested values as confirmed; hard-delete state only after authoritative absence; refuse duplicate, unowned, conflicting, membership-only, and shared historical-budget mutations; preserve historical imports while adding an unambiguous escaped form for colon-bearing team IDs; and emit bounded redacted diagnostics. ([#186](https://github.com/ncecere/terraform-provider-litellm/issues/186))
- **`litellm_team_member_add`**: Reconcile only explicitly owned members against LiteLLM v1.98.0's roster and membership rows; retain membership-only partial add/delete ownership with private canonical markers and hard manual-remediation diagnostics, then recognize administrator cleanup without state surgery; persist canonical IDs for email-only ownership; reject ambiguous legacy email-only or unresolved composite-import state; avoid v1.98's misleading 2xx membership-only update no-op; identify the current shared member budget only from team metadata; protect all historical shared-budget references; match email identity case-insensitively; preserve omitted-budget behavior; add collision-free escaped plain-team imports; and use bounded, narrowly classified redacted post-write retries. New configurations should prefer `for_each` with `litellm_team_member`. ([#184](https://github.com/ncecere/terraform-provider-litellm/issues/184))
- **`litellm_unified_access_group`**: Preserve the historical `assigned_key_ids` `list(string)` contract while reconciling normalized hashes as unordered membership; include only the intersection confirmed on both access-group and key rows; report one-sided read/import drift; safely repair all desired one-sided attach/detach directions using v1.98 delta behavior and exact hash-identified `/key/update` detaches without dropping unrelated groups or temporarily broadening authorization; retry empty accepted-create discovery; and restrict bounded verification retries to transient transport/HTTP categories while retaining peer-worker cache warnings. ([#189](https://github.com/ncecere/terraform-provider-litellm/issues/189))
- **Numeric state and JSON precision**: Preserve exact API integers across resources, list data sources, diagnostics, and the compiled legacy package; adopt Optional-only numeric values only during explicit imports; retain import ownership until endpoint envelopes and identities are authoritative; reject empty/null/malformed successful response bodies when a result is required, null numeric-map elements, and invalid JSON request configuration; and never replace an organization destructively to remove a per-model key. Organization lifecycle code may use only endpoint-supported, non-destructive complete replacement. ([#216](https://github.com/ncecere/terraform-provider-litellm/issues/216))
- **`litellm_credential` resource and data source**: Preserve the legacy `map(string)` types while loosening `credential_values` to optional for true model-only configuration and source-free imports; add canonical heterogeneous JSON objects; reject v1.98-invalid empty values-only creates; and align source dominance, exact routes, masks, recursive ownership, remote shape drift, import safety, and response validation with LiteLLM v1.98.0. Credential lookups now use bounded, capability-checked fresh-connection worker sampling so one process-local cache's exact 404 cannot remove state or permit duplicate create; prior-exact presence plus 404 supports apply followed by an immediate no-drift plan; idempotent fully hydrated PATCH fan-out tolerates only desired, prior, or absent cache results; terminal destroy distinguishes validated durable database deletion from unproven worker-cache revocation; and replacement deletion still requires consecutive all-404 sampling. ([#192](https://github.com/ncecere/terraform-provider-litellm/issues/192))
- **`litellm_organization_member`**: Align add, flat update, identity, exact role, authoritative read-back, nested budget, import, clear, not-found, and partial-failure behavior with LiteLLM v1.98.0. Configured member budgets now persist through the update endpoint after create. ([#185](https://github.com/ncecere/terraform-provider-litellm/issues/185))
- **List data sources**: Retrieve every `/key/list` and `/user/list` page with bounded churn retries, repeated-item and truncation guards; keep all historical IDs unchanged; use exact LiteLLM v1.98 filter aliases and escaped queries; preserve and canonicalize `/key/list` string/object SHA256 management hashes without rehashing or exposing malformed values; accept `/model/info` array and `user_model` object data; decode canonical endpoint shapes; and sort inventories deterministically. The budget inventory also exposes nullable reset and timestamp fields. ([#183](https://github.com/ncecere/terraform-provider-litellm/issues/183))
- **HTTP diagnostics**: Bound LiteLLM response reads and prevent API response bodies, secret-bearing URLs, request payloads, and transport details from leaking through Terraform or CI diagnostics. Safe errors retain only status, validated request IDs, and bounded redacted details; the compiled legacy package also stops logging payloads, URLs, and response bodies. ([#201](https://github.com/ncecere/terraform-provider-litellm/issues/201))
- **`litellm_access_group`**: Treat `model_names` as unordered LiteLLM membership while preserving its Terraform list type and configured ordering, accept duplicate and empty model names as LiteLLM v1.98 does, send sorted and deduplicated API payloads, preserve duplicate-containing configuration after deduplicated read-back, and surface real membership drift deterministically. Deployment-level `model_ids` remain deferred because LiteLLM v1.98 cannot read that identity back. ([#219](https://github.com/ncecere/terraform-provider-litellm/issues/219))
- **Request enum validation**: Synchronize key/team rate-limit types, internal-user and organization-member roles, MCP authentication, fallback type, prompt type, and model tier validators with LiteLLM v1.98.0 endpoint request contracts. Key limit types now accept `dynamic`; MCP accepts all v1.98 authentication modes; invalid legacy literals fail during planning without rewriting imported/read state, and bearer-token examples use the runtime-consumed `credentials.auth_value` key rather than ignored `token` or `api_key` keys. Team TPM/RPM limit types are create-only and force replacement when changed because LiteLLM ignores them on team updates. Recurring team and team-member budget durations accept positive `Ns`, `Nm`, `Nh`, `Nd`, or `Nw` values; the exact aliases `hourly`, `daily`, `weekly`, and `monthly`; or exactly `1mo`. Other month counts, zero values, case variants, and malformed aliases or units fail during planning. ([#220](https://github.com/ncecere/terraform-provider-litellm/issues/220))
- **`litellm_key_block`**: Replace raw-token resource IDs with automatically migrated SHA256 management IDs, use hash-only requests for the entire lifecycle, escape legacy query values, and suppress response bodies and transport details that may echo key material. ([#182](https://github.com/ncecere/terraform-provider-litellm/issues/182))
