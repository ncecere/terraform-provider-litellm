...
- **Request enum validation**: Synchronize key/team rate-limit types, internal-user and organization-member roles, MCP authentication, fallback type, prompt type, and model tier validators with LiteLLM v1.98.0 endpoint request contracts. Key limit types now accept `dynamic`; MCP accepts all v1.98 authentication modes; invalid legacy literals fail during planning without rewriting imported/read state. Team TPM/RPM limit types are create-only and force replacement when changed because LiteLLM ignores them on team updates. Recurring team-member budget durations accept positive `Ns`, `Nm`, `Nh`, or `Nd` values plus exactly `1mo`; unsupported aliases, week units, other month counts, zero values, and case variants fail during planning. ([#220](https://github.com/ncecere/terraform-provider-litellm/issues/220))
