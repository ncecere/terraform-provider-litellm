...
### Changed
- **Prompt read reliability (issue #202)**: Ordinary `litellm_prompt` resource refreshes now use bounded, cancellable retries for safe transient failures while preserving explicit environment scope. Because LiteLLM v1.98 uses ambiguous HTTP 400/404 responses for the singular route, the provider removes state only after one direct scoped versions read returns an exact typed 404. Successful responses project transactionally; create/update confirmation, data sources, list reads, absence probes, and mutations retain their existing single-attempt or endpoint-specific convergence behavior, and response-body text never establishes absence.
