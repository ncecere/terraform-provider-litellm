...
### Added
- **`litellm_jwt_key_mapping` resource and singular/list data sources**: Manage LiteLLM v1.98 JWT string claim-to-existing-virtual-key mappings with canonical UUID identity/import, write-only key creation, pre-mutation rejection of unverifiable rotation, immutable claim replacement, nullable description ownership/clear, explicit deactivate/reactivate, strict object/envelope validation, singular exact-404 absence proof, bounded double-scan pagination, source-valid empty claims, and redacted single-attempt failure recovery. ([#250](https://github.com/ncecere/terraform-provider-litellm/issues/250))
