...
### Fixed
- **`litellm_fallback` deletion authority**: Confirm remote absence after every DELETE, including a DELETE 404, and retain prior Terraform state with a bounded secret-safe error when LiteLLM still returns the fallback. This prevents pinned LiteLLM v1.98's non-authoritative DELETE behavior from silently orphaning active routing configuration. Successful deletion remains dependent on the upstream endpoint contract tracked in LiteLLM #38425. ([#264](https://github.com/ncecere/terraform-provider-litellm/issues/264))
