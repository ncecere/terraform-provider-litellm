...
- Release automation now fails closed unless the pushed tag has one exact, nonempty CHANGELOG section with explicit Upgrade Notes and Security subsections. CI uses pinned GoReleaser v2 tooling for nonpublishing configuration and all-target snapshot validation, while release builds no longer mutate module files. No provider schema, credential handling, or resource lifecycle behavior changes as part of #204. ([#204](https://github.com/ncecere/terraform-provider-litellm/issues/204))
