...
### Fixed
- **`litellm_organization` / `litellm_project`**: Read budget IDs, hard/soft budgets, duration, TPM/RPM, concurrency, and representable per-model controls authoritatively from v1.98's nested `litellm_budget_table` across resources and single/list data sources; preserve exact imported integers and omission provenance; validate relation/envelope identities; initialize and clear reset schedules; split project-row and budget updates with partial-failure safety; use transactional organization metadata replacement for removable per-model rate limits; and reject unsafe shared-budget creation, reassociation, or non-convergent clears. Structured `model_max_budget` data-source/organization surfaces remain deferred; the legacy project `map(float64)` field preserves existing state while rejecting inaccurate additions. Organization `blocked` and `tags` remain deprecated schema-compatible fields, but v1.98 phantom writes/reads are ignored and new unsupported non-default values are rejected. ([#188](https://github.com/ncecere/terraform-provider-litellm/issues/188))
