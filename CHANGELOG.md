...
### Added
- **`litellm_mcp_server` MCP info JSON ownership foundation (issue #213, stage 2)**: Add sensitive whole-document JSON, recursive selective override JSON, and canonical RFC 6901 clear-path controls; a non-sensitive ownership generation; schema v2 state migration; and value-free canonical private provenance with complete v1 compatibility. This stage validates and plans ownership only. Live JSON Create/Update/read hydration remains intentionally deferred to stage 3; the existing fixed `mcp_info` block and HTTP operation inventory are unchanged. ([#213](https://github.com/nicholas-cecere/terraform-provider-litellm/issues/213))
