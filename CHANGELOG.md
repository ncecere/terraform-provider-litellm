...
### Added
- **`litellm_team_member`**: New optional `budget_duration` attribute. The `/team/member_add` and `/team/member_update` endpoints accept only `max_budget_in_team` and no reset interval, so there is no way to give a team member a recurring budget via this provider. When set, the provider resolves the member's budget object via `/team/info` (by `budget_id`) and patches the duration through `/budget/update`; LiteLLM's reset job then resets the budget on that interval. Validated against `^[1-9][0-9]*(s|m|h|d|w|mo)$` and requires `max_budget_in_team` (the budget object the duration applies to). Clearing the attribute clears the duration; the call is skipped when the live duration already matches.
