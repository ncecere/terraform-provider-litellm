...
- **`litellm_credential` resource and data source**: Preserve the legacy `map(string)` types while loosening `credential_values` to optional for true model-only configuration and source-free imports; add canonical heterogeneous JSON objects; reject v1.98-invalid empty values-only creates; and align source dominance, exact routes, masks, recursive ownership, remote shape drift, import safety, response validation, bounded authoritative mutation postflights, exact 404 handling, and loss-preventing removal/replacement behavior with LiteLLM v1.98.0. ([#192](https://github.com/ncecere/terraform-provider-litellm/issues/192))
