...
### Added
- **`litellm_jwt_key_mapping` resource and singular/list data sources**: Manage LiteLLM v1.98 JWT string claim-to-existing-virtual-key mappings with canonical UUID identity/import, sensitive write-only key rotation, immutable claim replacement, nullable description clear, explicit deactivate/reactivate, strict object/envelope validation, exact-404 absence proof, deterministic complete pagination, API-owned import leaves, and redacted single-attempt failure handling. ([#250](https://github.com/ncecere/terraform-provider-litellm/issues/250))
