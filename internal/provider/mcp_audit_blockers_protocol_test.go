...
func TestMCPServerEqualRemoteOwnershipTakeoverAndUnknownRetentionProtocol(t *testing.T) {
	...
	t.Run("unknown owned values survive role masking", func(t *testing.T) {
		...
		state := accessGroupProtocolDynamicValue(t, schema, organizationProjectProtocolValue(t, schema, map[string]interface{}{
			...
			"env": map[string]tftypes.Value{"KNOWN": tftypes.NewValue(tftypes.String, "value")}, "env_vars": []tftypes.Value{}, "static_headers": map[string]tftypes.Value{"X-Known": tftypes.NewValue(tftypes.String, "value")},
		...
		unknowns := map[string]interface{}{
			...
			"env": tftypes.UnknownValue, "env_vars": tftypes.UnknownValue, "static_headers": tftypes.UnknownValue, "credentials": tftypes.UnknownValue, "allow_all_keys": tftypes.UnknownValue,
