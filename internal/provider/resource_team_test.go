...
func TestBuildTeamRequest_IncludesUserSuppliedTeamID(t *testing.T) {
	t.Parallel()
	ctx := context.Background()
	...
	data := &TeamResourceModel{
		TeamAlias:      types.StringValue("test-team"),
		TeamID:         types.StringValue("my-custom-team-id"),
		RouterSettings: types.ObjectNull(routerSettingsAttrTypes),
	}

	req := r.buildTeamRequest(ctx, data)

	if got, ok := req["team_id"].(string); !ok || got != "my-custom-team-id" {
		t.Fatalf("expected team_id 'my-custom-team-id' in request, got %T: %v", req["team_id"], req["team_id"])
	}
}

func TestBuildTeamRequest_OmitsTeamIDWhenUnset(t *testing.T) {
	t.Parallel()
	ctx := context.Background()

	r := &TeamResource{}

	// Null (not configured) and Unknown (to be generated by the API) must both
	// omit team_id from the request so the API assigns one.
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			req := r.buildTeamRequest(ctx, data)
			if _, ok := req["team_id"]; ok {
				t.Fatalf("team_id should be omitted when %s, got %v", tc.name, req["team_id"])
			}
		})
	}
}
