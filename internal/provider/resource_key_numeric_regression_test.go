...
func keyNumericProtocolValue(t *testing.T, schema *tfprotov6.Schema, computedUnknown bool, overrides map[string]tftypes.Value) tftypes.Value {
	t.Helper()
	attributes := make(map[string]tftypes.Value, len(schema.Block.Attributes))
	for _, attribute := range schema.Block.Attributes {
		value := interface{}(nil)
		if computedUnknown && attribute.Computed && !attribute.WriteOnly {
			value = tftypes.UnknownValue
		}
		attributes[attribute.Name] = tftypes.NewValue(attribute.ValueType(), value)
	}
	for name, value := range overrides {
		attributes[name] = value
	}
	return tftypes.NewValue(schema.ValueType(), attributes)
}

func keyNumericProtocolDynamic(t *testing.T, schema *tfprotov6.Schema, value tftypes.Value) *tfprotov6.DynamicValue {
	t.Helper()
	dynamic, err := tfprotov6.NewDynamicValue(schema.ValueType(), value)
	if err != nil {
		t.Fatalf("build protocol value: %v", err)
	}
	return &dynamic
}

func assertProtocolFloat64(t *testing.T, state *tfprotov6.DynamicValue, schema *tfprotov6.Schema, field string, want float64) {
	t.Helper()
	value, err := state.Unmarshal(schema.ValueType())
	if err != nil {
	...
	var got big.Float
	if err := attributes[field].As(&got); err != nil {
		t.Fatalf("decode %s: %v", field, err)
	}
	if got.Cmp(big.NewFloat(want)) != 0 {
		t.Fatalf("%s = %s, want %v", field, got.String(), want)
	}
}
...
func TestKeyNumericReadUsesFieldSpecificBudgetPrecedenceWithoutDrift(t *testing.T) {
	t.Parallel()
	...
	data.MaxBudget = types.Float64Value(100)
	data.SoftBudget = types.Float64Value(80)
	readKeyNumericResponse(t, response, &data, false)

	if data.MaxBudget.ValueFloat64() != 100 || data.SoftBudget.ValueFloat64() != 80 {
		t.Fatalf("field-specific budgets = %v, %v", data.MaxBudget.ValueFloat64(), data.SoftBudget.ValueFloat64())
	}
	...
	before := data
	readKeyNumericResponse(t, response, &data, false)
	if !data.MaxBudget.Equal(before.MaxBudget) || !data.SoftBudget.Equal(before.SoftBudget) || !data.TPMLimit.Equal(before.TPMLimit) || !data.RPMLimit.Equal(before.RPMLimit) || !data.MaxParallelRequests.Equal(before.MaxParallelRequests) {
		t.Fatalf("stable read drifted numeric state: before=%#v after=%#v", before, data)
	}
}

func TestKeyCreateAndDataSourceKeepVerificationTokenMaxBudget(t *testing.T) {
	t.Parallel()

	const key = "sk-live-budget-precedence"
	const infoResponse = `{
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		case request.Method == http.MethodPost && request.URL.Path == "/key/generate":
			_, _ = writer.Write([]byte(`{"key":"` + key + `"}`))
		case request.Method == http.MethodGet && request.URL.Path == "/key/info":
			_, _ = writer.Write([]byte(infoResponse))
		default:
	...
	if err != nil || accessGroupProtocolDiagnosticsHaveError(schemas.Diagnostics) {
		t.Fatalf("get schemas: err=%v diagnostics=%v", err, schemas.Diagnostics)
	}
	providerOverrides := map[string]tftypes.Value{
	...
	providerConfig := keyNumericProtocolValue(t, schemas.Provider, false, providerOverrides)
	configured, err := protocolServer.ConfigureProvider(ctx, &tfprotov6.ConfigureProviderRequest{
		TerraformVersion: "test",
		Config:           keyNumericProtocolDynamic(t, schemas.Provider, providerConfig),
	})
	...
	resourceSchema := schemas.ResourceSchemas["litellm_key"]
	resourceOverrides := map[string]tftypes.Value{
		"key":         tftypes.NewValue(tftypes.String, key),
		"max_budget":  tftypes.NewValue(tftypes.Number, 100),
		"soft_budget": tftypes.NewValue(tftypes.Number, 80),
	}
	resourceConfig := keyNumericProtocolValue(t, resourceSchema, false, resourceOverrides)
	proposed := keyNumericProtocolValue(t, resourceSchema, true, resourceOverrides)
	nullState := tftypes.NewValue(resourceSchema.ValueType(), nil)
	planned, err := protocolServer.PlanResourceChange(ctx, &tfprotov6.PlanResourceChangeRequest{
		TypeName:         "litellm_key",
		Config:           keyNumericProtocolDynamic(t, resourceSchema, resourceConfig),
		PriorState:       keyNumericProtocolDynamic(t, resourceSchema, nullState),
		ProposedNewState: keyNumericProtocolDynamic(t, resourceSchema, proposed),
	})
	if err != nil || accessGroupProtocolDiagnosticsHaveError(planned.Diagnostics) {
		t.Fatalf("plan key create: err=%v diagnostics=%v", err, planned.Diagnostics)
	}
	applied, err := protocolServer.ApplyResourceChange(ctx, &tfprotov6.ApplyResourceChangeRequest{
		...
		PlannedPrivate: planned.PlannedPrivate,
	})
	if err != nil || accessGroupProtocolDiagnosticsHaveError(applied.Diagnostics) {
		t.Fatalf("apply key create: err=%v diagnostics=%v", err, applied.Diagnostics)
	}
	assertProtocolFloat64(t, applied.NewState, resourceSchema, "max_budget", 100)
	assertProtocolFloat64(t, applied.NewState, resourceSchema, "soft_budget", 80)

	noDrift, err := protocolServer.PlanResourceChange(ctx, &tfprotov6.PlanResourceChangeRequest{
		TypeName:         "litellm_key",
		Config:           keyNumericProtocolDynamic(t, resourceSchema, resourceConfig),
		PriorState:       applied.NewState,
		ProposedNewState: applied.NewState,
		PriorPrivate:     applied.Private,
	})
	if err != nil || accessGroupProtocolDiagnosticsHaveError(noDrift.Diagnostics) {
		t.Fatalf("plan key no drift: err=%v diagnostics=%v", err, noDrift.Diagnostics)
	}
	priorValue, err := applied.NewState.Unmarshal(resourceSchema.ValueType())
	if err != nil {
		t.Fatal(err)
	}
	plannedValue, err := noDrift.PlannedState.Unmarshal(resourceSchema.ValueType())
	if err != nil {
		t.Fatal(err)
	}
	if !plannedValue.Equal(priorValue) {
		differences, _ := plannedValue.Diff(priorValue)
		t.Fatalf("stable key produced drift: %v", differences)
	}
	...
	dataSource, err := protocolServer.ReadDataSource(ctx, &tfprotov6.ReadDataSourceRequest{
		TypeName: "litellm_key",
	...
	if err != nil || accessGroupProtocolDiagnosticsHaveError(dataSource.Diagnostics) {
		t.Fatalf("read key data source: err=%v diagnostics=%v", err, dataSource.Diagnostics)
	}
	assertProtocolFloat64(t, dataSource.State, dataSourceSchema, "max_budget", 100)
	assertProtocolFloat64(t, dataSource.State, dataSourceSchema, "soft_budget", 80)
}
...
func TestKeyNumericFieldSpecificAuthoritativeNullsClearConfiguredState(t *testing.T) {
	t.Parallel()
	...
	readKeyNumericResponse(t, `{
		...
		"info":{
			...
			"litellm_budget_table":{"max_budget":100,"soft_budget":null,"model_max_budget":null},
...
func TestKeyDataSourcesUseFieldSpecificBudgetPrecedenceAndExactTopLevelLimits(t *testing.T) {
	t.Parallel()
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		_, _ = writer.Write([]byte(`{
			...
			"info":{
				...
				"rpm_limit":9223372036854775807,
				"max_parallel_requests":9007199254740997,
				"litellm_budget_table":{"max_budget":null,"soft_budget":80,"tpm_limit":1,"rpm_limit":2,"max_parallel_requests":3},
				"metadata":{},
				"blocked":false
			}
		}`))
	...
	if data.MaxBudget.ValueFloat64() != 100 || data.SoftBudget.ValueFloat64() != 80 || data.Spend.ValueFloat64() != 5.25 {
	...
	item, err := decodeKeyListItem(json.RawMessage(`{
		...
		"tpm_limit":9007199254740993,
		"rpm_limit":9223372036854775807,
		"litellm_budget_table":{"max_budget":null}
	}`))
	if err != nil {
